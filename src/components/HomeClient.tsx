...
export default function HomeClient({ trendingShows, popularShows }: Props) {
  const [selectedGenre, setSelectedGenre] = useState('all')
  const [filteredShows, setFilteredShows] = useState<TVShow[]>(popularShows)
  ...
  async function handleGenre(id: string) {
    ...
    try {
      if (id === 'all') {
        ...
        setFilteredShows(data.results?.slice(0, 6) ?? [])
      } else {
        ...
        <div className="flex gap-2 overflow-x-auto pb-1">
          {GENRES.map(g => (
            <button
              ...
              className={`flex-none px-4 py-1.5 text-xs font-bold font-label uppercase tracking-wide rounded-full transition-colors
                ${selectedGenre === g.id
      ...
      <section className="px-5 pt-4">
        ...
        {loadingGenre ? (
          ...
          <div className="grid grid-cols-2 gap-3 slide-up">
            {filteredShows.map(show => <PosterCard key={show.id} show={show} />)}
          </div>
        )}
      </section>
            ...
            return (
              <Link key={w.id} href={`/show/${w.id}`} className="flex-none w-28 group">
                <div className="relative w-28 h-40 rounded-2xl overflow-hidden bg-surface-container-high">
                  {p
                    ? <Image src={p} alt={w.name} fill className="object-cover group-hover:scale-105 transition-transform" sizes="112px"/>
                    : <div className="w-full h-full flex items-center justify-center"><span className="material-symbols-outlined text-outline">movie</span></div>}
                </div>
